...
class SqlTab(ttk.Frame):
    ...
    def _refresh_data_dictionary(self):
        ...
        desc_map = {
            ...
            # curriculum_stages
            ('curriculum_stages', 'stage_number'): 'Sequential stage number (1=first)',
            ('curriculum_stages', 'name'): 'Display name (e.g. "Wander")',
            ('curriculum_stages', 'description'): 'Flavor text shown in the editor',
            ('curriculum_stages', 'active_signals'): 'JSON list of reward signal names active in this stage',
            ('curriculum_stages', 'signal_scales'): 'JSON dict mapping signal name to scale (0 = silenced)',
            ('curriculum_stages', 'hunger_drain'): '1 = hunger depletes, 0 = creatures stay full forever',
            ('curriculum_stages', 'combat_enabled'): '1 = combat actions work, 0 = early-fail to noop',
            ('curriculum_stages', 'gestation_enabled'): '1 = eggs gestate and hatch, 0 = lifecycle frozen',
            ('curriculum_stages', 'mappo_steps'): 'Multi-agent PPO step count for this stage',
            ('curriculum_stages', 'es_generations'): 'ES generations (0 = skip ES phase)',
            ('curriculum_stages', 'es_variants'): 'Variants per ES generation',
            ('curriculum_stages', 'es_steps'): 'Sim steps per ES variant evaluation',
            ('curriculum_stages', 'ppo_steps'): 'Single-agent PPO step count',
            ('curriculum_stages', 'learning_rate'): 'Optimizer learning rate',
            ('curriculum_stages', 'ent_coef'): 'PPO entropy bonus weight',
            ('curriculum_stages', 'resume_from_stage'): 'Stage number to load weights from at start (NULL = fresh)',
            # item_frames
            ...
            ('tile_templates', 'liquid'): 'Whether this tile type is liquid (water, lava, etc.)',
            ('tile_templates', 'flow_direction'): 'Flow direction: N, S, E, W, or NULL for still',
            ('tile_templates', 'flow_speed'): 'Flow speed in tiles per second',
            ('tile_templates', 'depth'): 'Depth in tiles (0 = shallow/wading, 1+ = submerged for medium creatures)',
            ('tile_sets', 'liquid'): 'Override: liquid flag for this tile placement',
